	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("btn-reserva") != null) {
			...
			if (cva.getAlquiler().getFechaHasta().compareTo(fechaActual) < 0) {
				...
				totalAPagar = cva.getAlquiler().getPrecioAlquiler() + dias * cva.getVehiculo().getPrecio()
						+ dias * ((cva.getVehiculo().getPrecio() * 25) / 100);
				...
				totalAPagar = cva.getAlquiler().getPrecioAlquiler() + dias * cva.getVehiculo().getPrecio()
						+ dias * (cva.getVehiculo().getPrecio() * 50) / 100;
				...
				// si es devuelto el mismo día que termina el alquiler, el
				// compare da 0
				dias = (int) ((cva.getAlquiler().getFechaHasta().getTime() - fechaActual.getTime()) / 86400000);
